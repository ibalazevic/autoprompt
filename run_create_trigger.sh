...
for path in $datadir1/*; do
    ...
    if [ "$filename" = "P190" ] || [ "$filename" = "P20" ] || [ "$filename" = "P364" ] || [ "$filename" = "P37" ] || [ "$filename" = "P47" ] || [ "$filename" = "P740" ]; then
        echo "Creating trigger for $filename"
        $(CUDA_VISIBLE_DEVICES=1 python -m main.create_trigger $path out --model_name "roberta-base" --iters 50 --bsz 64 --patience 10 --num_cand 10 --format "X-5-Y" --use_ctx --synth > $logfile)
