...
for path in $datadir/*; do
    ...
    python -m main.create_trigger $path out --model_name "bert-base-cased" --iters 50 --bsz 64 --patience 10 --num_cand 10 --format "X-5-Y" > $logfile
...
for path in $datadir/*; do
    ...
    manual_prompt=$(sed -n ${i}p $manual_prompts_filename)
    echo "Creating trigger for $filename"
    python -m main.create_trigger $path out --model_name "bert-base-cased" --iters 50 --bsz 64 --patience 10 --num_cand 50 --format "X-5-Y" > $logfile
    echo "Saving results to $logfile"
    ((i++))
done
echo "--------------------------------------------------------------"
